...
- (void)initCell
{
    NSString *path = [[NSBundle mainBundle]pathForResource:[NSString stringWithFormat:@"%d",5] ofType:@"txt"];
...
//        NSString *path = [[NSBundle mainBundle]pathForResource:[NSString stringWithFormat:@"%d",arc4random()%4] ofType:@"txt"];
        [manager GET:@"http://0.0.0.0:5000/setting" parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
            //
            self.jsonStr = responseObject;
            [self.tableView reloadData];
